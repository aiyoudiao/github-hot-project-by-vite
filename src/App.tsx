...
import { HashRouter, Outlet, useLocation, useNavigate, useRoutes } from 'react-router-dom'
  ...
  return (
    <div className="px-12 flex flex-col h-screen bg-green-50 font-sans">
      <header className="py-4 flex items-center justify-between">
        <h1 className="text-3xl font-display font-bold text-primary">{headerText}</h1>
      ...
      <footer className="py-4 text-center text-primary-dark">群贤毕至，巅峰对决，谁与争锋？</footer>
    </div>
  )
})
...
const App: React.FC = () => {
  return (
    <HashRouter>
      <Suspense fallback={<div className="text-center text-primary">加载中...</div>}>
        <AppRoutes />
      </Suspense>
    </HashRouter>
  )
