 ...
 */
export function Icon({ className, iconName }: { className: string; iconName: string }): JSX.Element {
  if (!iconName) {
    return <></>
  }

  if (!IconMap[iconName] && typeof IconMap[iconName] === 'function') {
    return <FontAwesomeIcon className={className} icon={IconMap[iconName]}></FontAwesomeIcon>
  }
